...
///
/// \file
/// \brief Defines the vlang::SourceLocation class and associated facilities.
///
//===----------------------------------------------------------------------===//

#ifndef LLVM_VLANG_SOURCELOCATION_H
#define LLVM_VLANG_SOURCELOCATION_H
...
namespace llvm {
  ...
  /// DenseSets.
  template <>
  struct DenseMapInfo<vlang::FileID> {
    static inline vlang::FileID getEmptyKey() {
      return vlang::FileID();
    }
    static inline vlang::FileID getTombstoneKey() {
      return vlang::FileID::getSentinel();
    }

    static unsigned getHashValue(vlang::FileID S) {
      return S.getHashValue();
    }

    static bool isEqual(vlang::FileID LHS, vlang::FileID RHS) {
      return LHS == RHS;
    }
  };
  
  template <>
  struct isPodLike<vlang::SourceLocation> { static const bool value = true; };
  template <>
  struct isPodLike<vlang::FileID> { static const bool value = true; };

  // Teach SmallPtrSet how to handle SourceLocation.
  template<>
  class PointerLikeTypeTraits<vlang::SourceLocation> {
  public:
    static inline void *getAsVoidPointer(vlang::SourceLocation L) {
      return L.getPtrEncoding();
    }
    static inline vlang::SourceLocation getFromVoidPointer(void *P) {
      return vlang::SourceLocation::getFromRawEncoding((unsigned)(uintptr_t)P);
    }
