//===--- vlang/Basic/CharInfo.h - Classifying ASCII Characters ------------===//
//
...
#ifndef VLANG_BASIC_CHARINFO_H
#define VLANG_BASIC_CHARINFO_H
...
/// Returns true if this is a body character of an identifier,
// which is [a-zA-Z0-9_].
...
LLVM_READONLY static inline bool isPreprocessingNumberBody(unsigned char c) {
  using namespace charinfo;
  if ((InfoTable[c] & (CHAR_UPPER|CHAR_LOWER|CHAR_DIGIT|CHAR_UNDER|CHAR_PERIOD)) != 0)
     return true;
  return c == 'Z' || c == 'z' || c == 'X' || c == 'x' || c == '?';
...
LLVM_READONLY static inline bool isRawStringDelimBody(unsigned char c) {
  using namespace charinfo;
  return (InfoTable[c] & (CHAR_UPPER|CHAR_LOWER|CHAR_PERIOD|CHAR_DIGIT|CHAR_UNDER|CHAR_RAWDEL)) != 0;
}
