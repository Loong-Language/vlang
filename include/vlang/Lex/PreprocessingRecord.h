...
//
//===----------------------------------------------------------------------===//
#ifndef LLVM_VLANG_LEX_PREPROCESSINGRECORD_H
#define LLVM_VLANG_LEX_PREPROCESSINGRECORD_H
...
/// \brief Allocates memory within a Vlang preprocessing record.
void* operator new(size_t bytes, vlang::PreprocessingRecord& PR,
                   unsigned alignment = 8) throw();

/// \brief Frees memory allocated in a Vlang preprocessing record.
void operator delete(void* ptr, vlang::PreprocessingRecord& PR,
                     unsigned) throw();
...
namespace vlang {
  ...
  public:
    ...
    /// preprocessor.
    enum InclusionKind {
      /// \brief An \verilog \`include directive.
      Include
    };
...
inline void* operator new(size_t bytes, vlang::PreprocessingRecord& PR,
                          unsigned alignment) throw() {
  return PR.Allocate(bytes, alignment);
}

inline void operator delete(void* ptr, vlang::PreprocessingRecord& PR,
                            unsigned) throw() {
  PR.Deallocate(ptr);
}

#endif // LLVM_VLANG_LEX_PREPROCESSINGRECORD_H
