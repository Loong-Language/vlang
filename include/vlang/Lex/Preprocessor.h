...
#include "vlang/Lex/PPCallbacks.h"
...
class Preprocessor : public RefCountedBase<Preprocessor> {
  ...
  ///  if not expanding a macro.  This is an alias for either CurLexer.
  ...
  struct IncludeStackInfo {
    ...
    IncludeStackInfo(enum CurLexerKind K, Lexer *L, 
                     PreprocessorLexer* PPL,
                     TokenLexer* TL, const DirectoryLookup *D)
      : CurLexerKind(K), TheLexer(L), ThePPLexer(PPL),
  ...
  unsigned NumDirectives, NumIncluded, NumDefined, NumUndefined;
...
public:
  ...
  Systask::Context &getSystaskInfo() { return SystaskInfo; }
  ...
  /// that this ignores any potentially active macro expansions
  /// going on at the time.
  PreprocessorLexer *getCurrentLexer() const { return CurPPLexer; }

  /// getCurrentFileLexer - Return the current file lexer being lexed from.
  /// Note that this ignores any potentially active macro expansions
  /// going on at the time.
  ...
  /// \brief Recompute the current lexer kind based on the CurLexer/CurTokenLexer pointers.
...
public:
  ...
  /// returns true if the lexer should stop lexing tokens.  This is used
  /// for single expansion macros
...
private:
  ...
  /// We just read a \`ifdef or related directive and decided that the
  /// subsequent tokens are in the \`ifdef'd out portion of the
  /// file.  Lex the rest of the file, until we see an \#endif.  If \p
  /// FoundNonSkipPortion is true, then we have already emitted code for part of
  /// this \`ifdef directive, so \`else/\#`elsif blocks should never be entered. If
  ...
  bool InCachingLexMode() const {
    ...
    return CurPPLexer == 0 && CurTokenLexer == 0 &&
  ...
  void HandleTimescaleDirective(Token &Tok);
  ...
  void HandleElsifDirective(Token &Tok);
