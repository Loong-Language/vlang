...
void Lexer::InitLexer(const char *BufStart, const char *BufPtr,
  ...
  // to quickly lex the tokens of the buffer
  LexingRawMode = false;
...
void Lexer::resetExtendedTokenMode() {
  ...
  // TODO: Do we want to support keeping whitespace?
...
/// an escaped newline) then this count includes bytes that are part of that.
...
void Lexer::LexIdentifier(Token &Result, const char *curPtr){
	LexAnyIdentifier(Result,curPtr,false);
	return;
}

void Lexer::LexMacroIdentifier(Token &Result, const char *curPtr){
	LexAnyIdentifier(Result, curPtr, true);
	return;
}

void Lexer::LexAnyIdentifier(Token &Result, const char *CurPtr, bool isMacroReference) {
...
FinishIdentifier:
	...
	} else if (isMacroReference) {
	  // TODO: Error
	  Diag(CurPtr, diag::err_pp_visibility_non_macro) << II->getName();
	}
...
bool Lexer::SkipBlockComment(Token &Result, const char *CurPtr) {
  ...
  FoundSlash:
      ...
      if (CurPtr[0] == '*' && CurPtr[1] != '/') {
        // If this is a /* inside of the comment, emit an warning.  Don't do this
  ...
  // Notify comment handlers about the comment unless we're in a #if 0
  if (PP && !isLexingRawMode() &&
      PP->HandleComment(Result, SourceRange(getSourceLocation(BufferPtr),
                                            getSourceLocation(CurPtr)))) {
    BufferPtr = CurPtr;
...
/// LexTokenInternal - This implements a simple verilog family lexer.  It is an
...
void Lexer::LexTokenInternal(Token &Result) {
   if( BufferLast ) {
      //printf("%0s", llvm::StringRef(BufferLast, BufferPtr - BufferLast).str().c_str());
   }
   BufferLast = BufferPtr;
...
LexNextToken:
  ...
  case '\'':
     ...
     // Set as signed decimal to signal that it is a signed value
     if( Char == 'S' || Char == 's'){
       Kind   = tok::base_signed_decimal;
     ...
     switch(Char){
     case 'B': case 'b':
        Kind = (Kind == tok::base_signed_decimal) ? tok::base_signed_binary : tok::base_binary;
        CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
        break;
     case 'D': case 'd':
        Kind = (Kind == tok::base_signed_decimal) ? tok::base_signed_decimal : tok::base_decimal;
        CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
        break;
     case 'O': case 'o':
        Kind = (Kind == tok::base_signed_decimal) ? tok::base_signed_oct : tok::base_oct;
        CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
        break;
     case 'H': case 'h':
        Kind = (Kind == tok::base_signed_decimal) ? tok::base_signed_hex : tok::base_hex;
        CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
        break;
     default:
        if( Kind == tok::base_signed_decimal) {
           Diag(CurPtr, diag::err_invalid_base);
        }
        Kind = tok::quote;
        break;
     }
  ...
  case '!':
     if (getCharAndSize(CurPtr, SizeTmp) == '=') {
        ...
        case '=':
           CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
           Kind = tok::exclaimequalequal;
           break;
        case '?':
           CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
           Kind = tok::exclaimequalquestion;
           break;
        default:
  ...
  case '`':
    ...
    if (!LexingRawMode ) {
       ...
       // If it's an identifier, re-lex without the `
